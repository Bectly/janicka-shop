...
export interface CreditNoteData {
  ...
  // Totals (positive values — displayed as negative on PDF)
  refundAmount: number;
}
...
export function generateCreditNotePdf(data: CreditNoteData): Uint8Array {
  const doc = new jsPDF({
  ...
  doc.text("DOBROPIS", marginLeft, 25);

  doc.setFont("Inter", "normal");
  doc.setFontSize(11);
  doc.setTextColor(100, 100, 100);
  doc.text(`Číslo: ${data.creditNoteNumber}`, marginLeft, 33);
  ...
  y += 4;
  doc.text(
    `K faktuře: ${data.originalInvoiceNumber}`,
    marginLeft,
    y,
  );
  y += 4;
  doc.text(
    `Vratka: ${data.returnNumber}`,
    marginLeft,
    y,
  );
  y += 4;
  doc.text(
    `Důvod: ${data.returnReason}`,
    marginLeft,
    y,
  );
  ...
  const tableHead = [["Položka", "Počet", "Cena/ks", "Vráceno"]];
  ...
  doc.setFontSize(12);
  doc.setTextColor(180, 40, 40);
  doc.text("Celkem k vrácení:", totalsX - 45, y, { align: "right" });
  doc.text(`-${formatCzPrice(data.refundAmount)} Kč`, totalsX, y, {
    align: "right",
  });
  ...
  doc.text(
    "Tento dobropis je opravným daňovým dokladem dle §45 zákona č. 235/2004 Sb.",
    marginLeft,
    y,
  );
  ...
  doc.text(
    `Dobropis ${data.creditNoteNumber} · K faktuře ${data.originalInvoiceNumber} · Vystaveno ${formatCzDate(data.issuedAt)}`,
    pageWidth / 2,
    footerY + 4,
    { align: "center" },
  );
