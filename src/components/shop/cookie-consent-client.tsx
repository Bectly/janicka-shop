...
export function CookieConsentBannerClient({
  ...
  useEffect(() => {
    // Reconcile SSR decision with localStorage (authoritative client source).
    // SSR renders banner when no `cookie-consent` cookie is present; if the
    // user has saved consent in localStorage but the cookie expired/was
    // cleared, hide the banner post-hydration.
    // eslint-disable-next-line react-hooks/set-state-in-effect -- localStorage is only readable post-hydration
    if (getSavedConsent()) setVisible(false);
  }, []);
