...
## Tech Stack
- **Framework**: Next.js 16 (App Router) — upgraded from 15, currently on 16.2.2. Turbopack is default bundler. Cache Components (`"use cache"`) available. `middleware.ts` deprecated → `proxy.ts` (migration pending). **⚠️ C1511 SECURITY**: Two CVEs in 16.2.x branch (CVE-2026-27979 maxPostponedStateSize, CVE-2026-29057 http-proxy request smuggling in rewrites). Update to latest 16.2.x patch ASAP via `npm update next`.
...
- **Omnichannel recovery (NEW C1511)**: Email alone for cart recovery is no longer enough. **SMS cart recovery: 90%+ open rate**, response times <3 minutes. **Joyride SMS case study: $1.8M revenue, 12.4x ROI**, recovered 23,600+ orders. **WhatsApp recovery: 80% open rate, 5x ROI** (Picniq case study). Stores using **multi-channel (email + SMS + WhatsApp) get 2-3x higher engagement** than email alone. Top stores recover **15-30%** of abandoned carts through automation. **For CZ market**: WhatsApp widely used among women 18-35. Consider Resend for email + WhatsApp Business API for recovery sequence. SMS less preferred in CZ (cost per message higher than email/WhatsApp).
...
- **IMPORTANT (C31, verified C37, RE-VERIFIED C52, C1478, C1487, C1505, C1511)**: Checkout SDK still v2.0.15 (Nov 2025). **No new SDK version released as of April 4, 2026 (11th consecutive check).** Direct card number entry in SDK STILL "being prepared". Card payments continue to use redirect flow or inline iframe. @comgate/checkout-js-dev package exists on npm (v2.0.4, June 2025) but is an internal build variant, NOT the promised replacement. Comgate expanded to HU, PL, BG, RO, HR, AT, DE — growing internationally but SDK development still behind. **@comgate/checkout** is now officially deprecated — all functionality in `@comgate/checkout-js`.
