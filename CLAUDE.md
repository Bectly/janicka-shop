...
## Tech Stack
- **Framework**: Next.js 16 (App Router) — upgraded from 15, **currently on 16.2.3** (patched C2322). Turbopack is default bundler. Cache Components (`"use cache"`) available — **IMPLEMENTATION SPEC (C2720)**: (1) Add `cacheComponents: true` to `next.config.ts` top-level (NOT in `experimental`) — without this, `"use cache"` directives are silently ignored. (2) Syntax: `'use cache'; cacheLife('minutes'); cacheTag('products')` at top of async function. (3) Homepage/listing: `cacheLife('minutes')` + `cacheTag('products')`. (4) PDP: `cacheLife('minutes')` + `cacheTag(`product-${id}`)`. (5) For pages with `searchParams` (listing page is dynamic by default) — wrap the data-fetching helper function in `"use cache"` and pass params as serializable argument, NOT the page itself. (6) Invalidation from admin mutations: `import { revalidateTag } from 'next/cache'; revalidateTag('products', 'max')` — call from product create/update/hide/delete Server Actions. `middleware.ts` deprecated → `proxy.ts` (migration pending). **✅ C2322 SECURITY PATCHED**: All THREE CVEs fixed. CVE-2026-27979 + CVE-2026-29057 + CVE-2026-23869 (RSC DoS, CVSS 7.5 HIGH) — all fixed in 16.2.3 which is now installed.
...
## Competitive Landscape (Lead Research C19, UPDATED C34, C37, C1478, C1487, C1496, C1499, C1505, C2387, C2508, C2761)
- **Brumla.cz**: Largest CZ online second-hand. **59,386 items in stock** (C2508, up from 57,888), 15k new pieces/week. 98% Heureka (7,000+ reviews). Kids + women + men. **No mobile app, no AI, no personalization** — purely volume-based. Has reservation system (hold items 2 weeks, ship together). THREAT: LOW for our curated women's segment.
...
- **Vinted CZ (UPDATED C2761 — April 9, 2026 results)**: 75M+ members, **EUR 8B valuation**. **2025 ANNUAL RESULTS (published April 9, 2026): €10.8B GMV (+47% YoY), €1.1B revenue (+38% YoY), €62M net profit**. Vinted is now the **leading clothing retailer in France** by volume (ahead of Amazon). Expanded to Latvia, Estonia, Slovenia. Vinted Go logistics now in 5 markets. **Testing US market**. IPO still ruled out. Vinted Pay wallet live in 8 EU countries — **CZ still NOT included**, now live in Slovakia (CZ rollout possible within 2026). Collections now paid promotional tool. **AI CRISIS DEEPENING (C1496+C1505)**: OECD formally catalogued GenAI refund fraud. **65% of consumers say AI has made false refund claims easier** (Ravelin). **C1499 TRUSTPILOT**: Vinted.cz = **2.1/5**, **76% one-star reviews** (unchanged C2298). CZ users escalating to **ČOI** + European Consumer Centre. Trust crisis DEEPENING. **⚠️ C2298 T&C MARKETING WINDOW**: Forced T&C update **April 30, 2026** — AI training clause is **non-opt-out, perpetual, worldwide** license for user photos/listings to train AI models. Settings opt-out covers only marketing campaigns, NOT AI training. Exact T&C language: *"adapt Content for...development and improvement of artificial intelligence (AI) and machine learning models and algorithms."* Duplicate-listing crackdown active — sellers losing 80% of views. **User quote (organic): "Ever since Vinted switched to AI, accounts are being blocked for no clear reason."** Marketing window: April 28–May 1. Janicka copy: *"Tvoje fotky jsou tvoje. Nikdy je nepoužijeme k trénování AI."* **✅ C2387: Trust messaging LIVE** — deployed C2379, CZ media still zero coverage after **29th consecutive scan** (C2788, April 9, 2026). **19 days left** until April 28 window open. Exclusive competitive advantage currently live on homepage. Marketing window: April 28–May 1. **⚡ C2788 CAMPAIGN BRIEF**: Email newsletter send April 28 17:00 CET via Resend admin panel. Subject A (warm list): *"Tvoje fotky patří tobě. Vždy."* Subject B (cold/less-engaged): *"Zatímco Vinted školí AI na tvých fotkách..."*. Preview text: *"U nás je to jinak. A vždy bylo."* Body: contrast Janicka vs Vinted AI clause → CTA to shop. Second send optional April 30 09:00 CET (enforcement deadline). **✅ Task #54**: Dedicated `/soukromi` trust landing page added for Bolt — shareable page for email CTA + Instagram bio link + social sharing during campaign window. **Scale note**: Vinted's €10.8B GMV proves the second-hand market is massive — but Vinted's trust crisis (AI scams, T&C AI training, 2.1/5 Trustpilot) creates the differentiation window for curated alternatives like Janicka.
...
- **Client SDK**: `@comgate/checkout-js` v2.0.15 (npm, last published Nov 2025) — replaces old `@comgate/checkout`. TypeScript with bundled types, promise-based API, framework-agnostic. ⚠️ Will be replaced by new version (no ETA). Wrap calls for easy migration. **31st consecutive check (C2720, April 9, 2026) — still v2.0.15. 6+ months frozen. No ETA from Comgate.**
...
- **Packet creation (C2496 Scout CONFIRMED — SOAP ONLY)**: `https://www.zasilkovna.cz/api/soap` (WSDL: `?wsdl`). **REST v5 (`api.packeta.com/v5`) is pickup-point queries only — NOT for packet creation.** Use `npm install soap` or raw XML POST. Required: `number` (order ID), `name`, `surname`, `value` (CZK), `weight` (kg, min 0.001), `addressId` (numeric point.id from widget), `eshop` (sender label). Optional: `email`, `phone`, `cod`. `createPacket` returns `packetId` — store on Order model.
- **Labels**: `packetsLabelsPdf(apiPassword, [packetId], "A6 on A4", 0)` → base64 PDF. Decode and stream to admin or save to R2.
- **Status tracking**: `packetStatus(apiPassword, packetId)` → `codeText`, `storedUntil`, `isReturning`.
- **Cancel**: `cancelPacket(apiPassword, packetId)`.
- **Pricing**: Contract-based, no public API — set fixed shipping price in shop (typicky 69-89 Kč).
- **No official Node SDK**. Use `soap` npm package or raw XML. Server Action only — `apiPassword` must stay server-side.
- **Gotcha**: weight=0 rejected. `addressId` is the numeric string from widget `point.id`. Sandbox: contact Packeta for separate sandbox credentials (not documented publicly).
...
- **EU GPSR (General Product Safety Regulation) (UPDATED C2694 Scout research)**: In effect since **December 13, 2024**. **NO micro-enterprise exemption** (unlike EAA). Czech implementing Act No. 387/2024 Coll., max fine CZK 50M. **ČOI enforcement NOT yet active for clothing** — priority is electronics/toys. **For second-hand reseller (distributor role)**: Full obligation is to verify manufacturer info exists, NOT to republish it on every page. **Practical compliance path**: (1) Display brand name (✅ already done) + brand's official website URL as "electronic address" — satisfies Article 19 distance-selling requirement. (2) Populate Heureka `MANUFACTURER_ELECTRONIC_ADDRESS` with brand website URL (e.g. `https://www.zara.com`) — low effort, Heureka tags are currently optional but protect against future enforcement. (3) Heureka `MANUFACTURER_POSTAL_ADDRESS` — optional for now; add brand's registered EU address for top brands (Zara=Inditex Spain) if time permits. **NOT required for standard clothing**: CE marking, Declarations of Conformity, batch/serial numbers. **Product photos showing brand labels** satisfy product identification requirement. No new fields needed in Product model for launch compliance.
...
- **Google Merchant Center XML feed**: `/api/feed/google-merchant` endpoint with full GMC spec (g: namespace RSS 2.0). ✅ DONE (C2310). **Remaining**: register at merchants.google.com, connect domain, submit feed URL for free organic Shopping listings. **⚠️ URGENT C2370 Scout**: Google is folding its Doppl VTO experiment into Google Shopping listings ~April 30. Products indexed in GMC **may get free "Try It On" in Google Search** — zero cost to merchant. GMC registration is now time-sensitive (April 30). Image quality matters for VTO eligibility: front-facing, clean background, model wearing item preferred.
