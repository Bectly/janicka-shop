...
- [~] Cloudflare Load Balancer setup (Hetzner primary, Vercel backup) — runbook `docs/migration/runbooks/p6.1-cloudflare-load-balancer.md` + config blueprint `docs/migration/cloudflare-lb.md` both ready-to-execute on apex `jvsatnik.cz` (zone already active — same as `jarvis-janicka.jvsatnik.cz` tunnel). Session affinity `ip_cookie` TTL 1800s per directive. Blocked on P3.4 Hetzner smoke-test (#334) + LB paid add-on activation
...
- **Cycle #4380 (Bolt, task #341 P6.1)** — Reconciled `docs/migration/runbooks/p6.1-cloudflare-load-balancer.md` with the newer config blueprint `docs/migration/cloudflare-lb.md` + `dns-shift.md`. Apex hostname flipped from stale `janicka-shop.cz` → `jvsatnik.cz` (the domain bectly already owns — same CF zone as `jarvis-janicka.jvsatnik.cz` Tunnel). Session affinity row flipped from `OFF` → `ip_cookie` TTL 1800s + `samesite=Lax`, `secure=Always`, `drain_duration=60s`, `zero_downtime_failover=sticky` per C4378 directive (belt-and-suspenders against mid-checkout bouncing across origins + Vercel cold-start). Pre-flight checklist drops the domain-purchase gate; remaining blockers: P3.4 Hetzner smoke-test (#334) + LB paid add-on activation. Zero code changes; runbook + progress log only. task_id 341.
