...
| W-9 (NEW C4916) — `globalTeardown` does not revert `Product.createdAt` mutations from `#565 back-in-stock-dispatch.spec.ts` | ✅ **OBVIATED at C4921, documented at C4922 (#570 Bolt)**. The W-12 spec rewrite restructured `back-in-stock-dispatch.spec.ts` to create its OWN dedicated Product whose default `createdAt = now()` already sits inside the cron's 48h-fresh window — the post-create `prisma.product.update({ data: { createdAt: … } })` mutation that originally seeded this gap was deleted as part of the per-spec product reservation strategy. With no createdAt mutation in any e2e spec, no teardown logic is required. Tracked in `scripts/playwright-global-teardown.ts` header comment so future maintainers don't reintroduce a createdAt mutation without re-opening the gap. |
| W-9b (NEW C4916) — orphan `WishlistSubscription` / `BackInStockSubscription` / `EmailDedupLog` rows on SIGKILL | ✅ **CLOSED at C4922 (#570 Bolt)** in `scripts/playwright-global-teardown.ts`. Added per-table `deleteMany({ where: { AND: [{ email: { contains: '-e2e-' } }, { email: { endsWith: '@test.local' } }] } })` sweeps for `emailDedupLog`, `wishlistSubscription`, `backInStockSubscription`. Email shape `<prefix>-e2e-<unique>@test.local` (matches `wishlist-sold-e2e-…@test.local` + `back-in-stock-e2e-…@test.local`). Each sweep logs its row count when non-zero. |
| W-9c (NEW C4916) — `#563 wishlist-sold` flake on sparse dev DB (single product in category → no similarProducts → no dedup row → assertion fails) | ✅ **CLOSED at C4922 (#570 Bolt)** in `e2e/wishlist-sold-trigger.spec.ts:beforeAll`. Replaced `prisma.category.findFirst` with `prisma.product.groupBy({ by:["categoryId"], where:{ active:true, sold:false }, having:{ categoryId:{ _count:{ gte:1 } } }, take:1 })` — the cron requires ≥1 OTHER active+unsold product in the category to populate `similarProducts` and write the dedup row, so the groupBy filters out sparse categories at the source. Mirrors the `sold-pdp.spec.ts` `groupBy({ having: _count.gte:4 })` precedent. test.skip message updated to "No category with ≥1 active+unsold product in dev DB". |
| W-10 (NEW C4916) — `globalTeardown` does not sweep `E2E Test Product %` orphan rows from `#566 admin-product-create` | ✅ **CLOSED at C4922 (#570 Bolt)** in `scripts/playwright-global-teardown.ts`. Added `prisma.product.deleteMany({ where: { AND: [{ sku:{ startsWith:'E2E-' } }, { name:{ startsWith:'E2E Test Product ' } }] } })` with PriceHistory cascade-delete first (FK satisfaction). Double-key on both columns neutralises false-positive risk if a real seller ever uses an `E2E-` SKU prefix. Sweep covers all three new specs (admin-product-create + the W-12 reserved products from wishlist-sold-trigger / back-in-stock-dispatch / sold-pdp, all of which carry the `E2E-` SKU + `E2E Test Product ` name prefix). |
| W-11 (NEW C4916) — `#566 admin-product-create` listing-pagination assumption | ✅ **CLOSED at C4922 (#570 Bolt)** in `e2e/admin-product-create.spec.ts`. Replaced `await page.goto("/products")` with `await page.goto("/products?sort=newest")` — pins the sort param explicitly so the listing-link assertion does not silently regress if the listing default sort ever changes (currently `newest` per `products-client.tsx:94`, but explicit > implicit for regression guards). PDP slug assertion (line 124-129) remains as the primary integrity check. |
...
- **W-row queue net**: 1 closed (W-5), 5 new (W-9, W-9b, W-9c, W-10, W-11, W-12). **W-12 is the only P2** in the new set — others are P3. **Net post-C4922**: Phase 8 W-row queue fully drained — W-12 (C4921), W-9 (obviated C4921, documented C4922), W-9b / W-9c / W-10 / W-11 (all CLOSED C4922 #570 single-PR Bolt landing per Trace recommendation).
- **Phase 8 verdict**: e2e bundle landed clean on `tsc/lint/vitest` axes; **runtime e2e flake risk is moderate** (W-12 race + W-9c sparse-DB skip). Recommend Bolt next cycle prioritise **W-12 first** (P2, intermittent test failures will erode trust in the suite faster than dev-DB orphan rows), then W-9 + W-10 + W-9b as a single `globalTeardown` extension PR (cheap one-route addition: revert createdAt + delete e2e orphan rows + delete e2e orphan products). **Status post-C4922**: this recommendation has been executed in two cycles — C4921 closed W-12 (highest priority), C4922 closed W-9b / W-9c / W-10 / W-11 in a single bundled landing (#570) and obviated W-9 via the W-12 spec rewrite. Phase 8 e2e teardown class is now resilient to SIGKILL across all sweep dimensions: sold-flag revert (W-5), e2e orphan products (W-10), e2e orphan subscription/dedup rows (W-9b), and sparse-DB-skip (W-9c).

### Findings summary by P-tier

- **P0** (security/data-loss): **none**.
- **P1** (quality blockers): **none new**. Carries: P (ts-prune close-out).
- **P2** (quality wins): **W-12 NEW** (cross-spec race under fullyParallel — highest priority). Carries: N, O, M. **W-12 CLOSED at C4921 (#569).**
