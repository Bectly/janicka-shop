...
## 10. J8 — Full-field progressive disclosure (Cycle #5045)

**Directive #780**: Mobile QR UI must support ALL Product fields. Janička adds 5-20 pieces per session — she must NOT need to come back to PC to fill gaps.

Tasks: #804 (Bolt/schema), #805 (Bolt/brand-API), #806 (Sage/base-mobile), #807 (Sage/progressive), #808 (Trace/E2E)

### 10.1 Default view (6 fields always visible)

```
┌─────────────────────────────────────┐
│  📸 Batch #cmXYZ  · 3 kousků       │
│                                     │
│  ┌─────────────────────────────┐    │
│  │  📷  Vyfotit / z galerie    │    │  ← full-width tap target
│  └─────────────────────────────┘    │
│  [🖼 thumb][🖼 thumb][+]            │  ← swappable order
│                                     │
│  Název ________________________     │  ← text + long-press = voice
│                                     │
│  Cena  _______ Kč                   │  ← numeric pad
│                                     │
│  Stav                               │
│  [●NŠ][NB][Výb][Dob][Vad]          │  ← segmented, 5 options
│                                     │
│  Kategorie                          │
│  [📌Trička][📌Kalhoty][Svetry][+]   │  ← last 3 pinned from localStorage
│                                     │
│  Velikost  (smart by category)      │
│  [XS][S][●M][L][XL][XXL]           │  ← clothes: XS-XXL / shoes: 35-46
│                                     │
│  ──────────────── [Více detailů ▼]  │
│                                     │
│  ┌─────────────────────────────┐    │
│  │   Uložit a další   →        │    │  ← sticky bottom CTA
│  └─────────────────────────────┘    │
│         [Hotovo — zavřít batch]     │
└─────────────────────────────────────┘
```

### 10.2 "Více detailů" expanded

```
│  ── Více detailů ─────────────────  │
│                                     │
│  Značka _________________ [🔍]      │  ← autocomplete /api/admin/brands/suggest
│                                     │
│  Barvy                              │
│  [●čer][mod][zel][bíl][čer][šed]   │  ← visual color chips
│  [hně][béž][rů][fial]              │
│                                     │
│  ▶ Rozměry (cm)  [rozbalit]         │
│  │  Hrudník ___  Pas ___            │  ← collapsed by default, tap to expand
│  │  Boky ___     Délka ___          │
│                                     │
│  Poznámka k střihu ____________     │  ← fitNote, max 120 chars
│  "Sedí spíš na štíhlejší"          │
│                                     │
│  ── Pro pokročilé ─────────────────  │
│                                     │
│  Původní cena ___ Kč (přeškrtnutá) │  ← compareAt
│  [☐] Zvýraznit na homepage          │  ← featured toggle
│  Interní poznámka ______________    │  ← internalNote (admin-only)
│  SEO titulek ___________________    │  ← metaTitle (≤70)
│  SEO popis _____________________    │  ← metaDescription (≤160)
└─────────────────────────────────────┘
```

### 10.3 Condition-triggered defect section (auto-expand)

When Stav ∈ {Dobrý, Viditelné opotřebení}:

```
│  ┌─ Vady (povinné pro zvolený stav) ┐  │
│  │  Popis vad _________________   │  │  ← defectsNote textarea
│  │  [📷 Vyfotit vadu]  [🖼][🖼]  │  │  ← defectImages
│  └───────────────────────────────┘  │
```

### 10.4 Voice-to-text pattern

Long-press (≥500ms) on any text field → `SpeechRecognition` (Web Speech API):

```
│  Název [🎤 ● REC — mluvte nyní...] │  ← pulsing red dot while active
│  "Dámský svetr Zara bílý"          │  ← live transcript preview
```

Graceful fallback: if `window.SpeechRecognition` undefined → no mic icon shown.

### 10.5 Per-category defaults cache (localStorage)

Key: `janicka_category_defaults` → `{[categoryId]: {fitNote, measurements, lastUsedAt}}`

After 3 items in same category, show:

```
│  [💾 Použít stejné jako minule]     │  ← one-tap pre-fill fitNote+measurements
```

### 10.6 Field → DB mapping (complete)

| Mobile UI field         | ProductDraft field    | Product field       | Source       |
|-------------------------|-----------------------|---------------------|--------------|
| Fotky                   | images[]              | images[]            | R2 upload    |
| Název                   | name                  | name                | direct       |
| Cena                    | price                 | price               | direct       |
| Stav                    | condition             | condition           | direct       |
| Kategorie               | categoryId            | categoryId          | chip select  |
| Velikost                | sizes[]               | sizes[]             | chip multi   |
| Značka                  | brand                 | brand               | autocomplete |
| Barvy                   | colors[]              | colors[]            | chip multi   |
| Rozměry                 | measurements {}       | measurements {}     | accordion    |
| Poznámka k střihu       | fitNote               | fitNote             | text ≤120    |
| Popis vad               | defectsNote           | defectsNote         | textarea     |
| Foto vad                | defectImages[]        | defectImages[]      | R2 upload    |
| Původní cena            | compareAt             | compareAt           | numeric      |
| Zvýraznit               | featured              | featured            | toggle       |
| Interní poznámka        | internalNote          | internalNote        | textarea     |
| SEO titulek             | metaTitle             | metaTitle           | text ≤70     |
| SEO popis               | metaDescription       | metaDescription     | text ≤160    |
| Hmotnost (g)            | weightG               | weightG             | numeric opt. |
| — (from batch)          | —                     | bundleId            | batch.bundleId |
| — (from batch)          | —                     | bundleLineId        | batch.bundleLineId |

*Video upload (videoUrl) deferred to J8-S3 — requires UploadThing video endpoint.*
